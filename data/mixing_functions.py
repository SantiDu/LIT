...
Parts of this code were taken from IMA 
https://github.com/lgresele/independent-mechanism-analysis/tree/master/ima
...
def darmois_linear_gaussian_2d(A):
    ...
    def darmois(x):
        ...
        y_1 = 0.5 * (1.0 + special.erf(
            (x[1] - c_1_given_0 * x[0]) /
            jnp.sqrt(2 * (1.0 - rho_01**2) * sigma_1**2)))
        return jnp.array([y_0, y_1])

    def inv_darmois(y):
        s_0 = sigma_0 * jnp.sqrt(2) * special.erfinv(2.0 * y[0] - 1.0)
        s_1 = jnp.sqrt(2 * (1.0 - rho_01**2) * sigma_1**2) * special.erfinv(
            2.0 * y[1] - 1.0) + c_1_given_0 * s_0
