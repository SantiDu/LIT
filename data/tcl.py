...
Data generation in the format of time-contrastive learning (TCL)
...
def generate_artificial_data(num_comp,
    ...
    source, label, L = gen_source_tcl(num_comp,
                                      num_segment,
                                      num_segmentdata,
    ...
    # Apply mixing MLP
    sensor, mixlayer = apply_MLP_to_source(source,
                                           num_layer,
    ...
    if num_segmentdata_test is not None:
        source_test, label_test, L_test = gen_source_tcl(
            num_comp,
            num_segment,
            num_segmentdata_test,
            L=L,
            random_seed=random_seed + 1)  # change random_seed
        # Use same parameters of the training data
        sensor_test, mixlayer_test = apply_MLP_to_source(
            source_test,
            num_layer,
            num_segment=num_segment,
            random_seed=random_seed)

    return sensor.T, source.T, label
...
def gen_source_tcl(num_comp,
    ...
    if L is None:
        if Ltype == "uniform":
            L = np.random.uniform(min(Lrange), max(Lrange),
                                  [num_comp, num_segment])
        else:
            raise ValueError(
                '{} type for L factor not implemented'.format(Ltype))
    ...
    for sn in range(num_segment):  # Segment
        for cn in range(num_comp):  # Component
            ...
            if sourcetype == 'laplace':
                source_sc = np.random.laplace(0, 1 / np.sqrt(2),
                                              [1, num_segmentdata])
                source_sc = source_sc * L[cn, sn]
            elif sourcetype == 'gaussian':
                source_sc = np.random.normal(size=[num_segmentdata])
                source_sc = source_sc * L[cn, sn]
            elif sourcetype == 'combo':
                source_sc = np.random.laplace(0, 1 / np.sqrt(2),
                                              [1, num_segmentdata])
                source_sc = source_sc * (
                    np.random.normal(size=[num_segmentdata]) + L[cn, sn])
            elif sourcetype == 'uniform':
                source_sc = np.random.uniform(low=-1,
                                              high=1,
                                              size=[1, num_segmentdata])
                source_sc = source_sc * L[cn, sn]
            else:
                raise ValueError(
                    '{} source type not implemented'.format(sourcetype))
    ...
    for ln in range(num_layer - 1, -1, -1):
        ...
        if ln < num_layer - 1:  # No nolinearity for the first layer (source signal)
            if nonlinear_type == "ReLU":  # Leaky-ReLU
                mixedsig[
                    mixedsig < 0] = negative_slope * mixedsig[mixedsig < 0]
            else:
                raise ValueError(
                    '{} nonlinear activation not implemented for mixing'.
                    format(nonlinear_type))
        ...
        # Bias
        b = np.random.uniform(Arange[0], Arange[1],
                              [num_comp]).reshape([1, -1]).T
...
if __name__ == '__main__':
    generate_artificial_data(2, 10, 20, 2)